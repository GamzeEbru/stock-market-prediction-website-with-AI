...
const DataDetails = (props) => {
  ...
  {/* historical data pagination begin*/}
  const lastPostIndex = currentPage * postsPerPage;
  const firstPostIndex = lastPostIndex - postsPerPage;
  const currentPosts = data.slice(firstPostIndex, lastPostIndex);
  const upperTableName = tableName.toUpperCase();
  {/* historical data pagination end*/}
        ...
        <div>
          <div>
            <p className="font-sans hover:font-bold text-3xl ml-8 p-8 text-stone-700">
              {tableName.toUpperCase()} PREDİCT DATA
            </p>
          </div>
                ...
                {currentPosts.map((row, index) => (
                  <tr key={index} className="bg-white border-b">
                    ...
                    <td className="px-6 py-4">                  
                      {formatDecimal(row.Tahmin - row.Kapanış)}
                    </td>
                  </tr>
                ))}
              </tbody>       
            ...
            <table className="w-full text-sm text-left text-gray-500">
              ...
              {currentPosts.map((row, index) => (
                <tr key={index} className="bg-white border-b">
                  ...
                  <td className="px-6 py-4">{formatDecimal(row.Kapanış)}</td>
                  <td className="px-6 py-4">{formatDecimal(row.AdjClose)}</td>
